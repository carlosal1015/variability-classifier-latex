...
\todo{Add information about number of data points per band}
\todo{Mention that $B_E$ is better sampled}
\todo{Explain the preprocessing: cut out placeholder data points}
\todo{Mention that no $3\sigma$ clipping was performed}
% Mention that B_E is better sampled

\todo[inline]{Insert a table describing the different abbreviations, especially for subclasses}
...
\litem{$Q_{25}, Q_{25}^\phi$ ($25^\text{th}$ percentile)} The $25^\text{th}$ percentile of the magnitude distribution, given by
\begin{equation}Q_{25} = \cdots.\end{equation}

\litem{$Q_{75}, Q_{75}^\phi$ ($75^\text{th}$ percentile)} The $75^\text{th}$ percentile of the magnitude distribution, given by
...
\begin{equation}\text{MAD}_{Q_{50}} = .\end{equation}\todo{Add equation}
...
\litem{$\mathcal{F}$ (Fourier decomposition)} We fit standard fourier series with five terms to the phase--folded light curve.
...
\litem{$T_{\text{CE}_1}, T_{\text{CE}_2}, T_{\text{CE}_3}$ (CE periods)} We add three candidate periods with the lowest conditional entropy according to the CE periodogram, as introduced in section \ref{subsubsec:conditional-entropy}\todo{Mention filtering of aliases}. Furthermore, we also add the CE scores, $\text{CE}_1, \text{CE}_2, \text{CE}_3$, for each of the periods as features.
...
\todo{Check proper citing of all equations}

\end{enumerate}

This results in 64 features per band ($B_R, B_B, B_{R-B}$), so 192 features overall.\todo{Note that features are highly correllated}
...
\todo[inline]{Add some histograms of the features}
\todo[inline]{Add plots containing periodogram for LS and CE}
...
\improvement[inline]{Add figure for feature importance plot from RF}
