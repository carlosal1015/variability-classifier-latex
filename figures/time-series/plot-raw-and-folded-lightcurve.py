...
for filename in options.lightcurves:
	...
	if options.name:
		figurepath = os.path.join(options.name + '-folded' + '.png')
	else:
		figurepath = os.path.join(filename.split('.time')[0] + '-folded' + '.png')
...
ax.errorbar(t, m, yerr = dm, fmt = '.k', ecolor = 'gray', label = filename.replace('.time', ''))
ax.set_ylim((m.mean() - 2.5 * m.std(), m.mean() + 2 * m.std()))
ax.set_xlabel('$t$ [$\mathrm{d}$]', size = 38)
if options.band == 'r':
	ax.set_ylabel('$m$ ($R_E$)', size = 36)
else:
	ax.set_ylabel('$m$ ($B_E$)', size = 36)
